import mqtt from '../../utils/mqtt.js';//MQTT库

//连接的服务器域名，注意格式！！！
const host = 'wx://192.168.31.236/mqtt';//wx://会被解析为ws://,wxs://会被解析为wss://
Page({
  data: {
    client: null,
    //记录重连的次数
  ...
  onClick_PubMsg: function() {
    if (this.data.client && this.data.client.connected) {
      this.data.client.publish('ZT', 'on');
      wx.showToast({
        title: '发布成功'
      })
    } else {
  ...
  offClick_PubMsg: function() {
    if (this.data.client && this.data.client.connected) {
      this.data.client.publish('ZT', 'off');
      wx.showToast({
        title: '发布成功'
      })
    } else {
